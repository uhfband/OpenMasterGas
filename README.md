...
На запрос со статусом пульт должен ответить, иначе котел прекратит работу.
На запрос второго типа можно не отвечать.  
...
```
Контрольная сумма считается следующим образом: складываются 6 байт пакета, затем сумма инвертируется, и прибавляется 1.
```

## Схема WiFi адаптера и скетч
